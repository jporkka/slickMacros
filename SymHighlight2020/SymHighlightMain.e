...
static int s_modtimeSymHighlight2 = 0; // Tracks changes to highlights and updates buffers with out of date highlights.
...
void sym_update_screen(boolean fNow, _str updateMethod="sym_symtag_update_window")
{
    ...
    if (gSymAutoUpdate)
    {
        ...
        //say("< " copymodTime ", s_modtimeSymHighlight=" s_modtimeSymHighlight);
        //say("< " p_LastModified ", s_modtimeSymHighlight=" s_modtimeSymHighlight2);
        //sym_symtag_update_window_wrapper(updateMethod);
        //for_each_mdi_child(sym_symtag_update_window_wrapper,updateMethod);
...
void _switchbuf_tbhighlight(_str oldbuffname, _str flag, _str swold_pos=null, _str swold_buf_id= -1)
{
    ...
    //say("-------------------");
    //say("modTime:" copymodTime \
    //    ", s_modtimeSymHighlight:" s_modtimeSymHighlight \
    //    ", SwitchBuf, Old:" oldbuffname \
    //    ", New:" p_buf_name \
    //    ", lastMod:" p_LastModified);
...
static int mouseClickHigh(int MarkerIndex)
{
    say("mouseClickHigh index=" MarkerIndex);
    return 0;
}
                ...
                if (s != "")
                {
                    ...
                    if (c != null)
                    {
                        ...
                        if (colorIndex != -1)
                        {
                            ...
                            //say("Set mark offset=" offset_highlight ", index=" pos_marker);
                            //int xx = _MarkerTypeSetCallbackMouseEvent(s_markertypeSymHighlight, LBUTTON_DOWN, mouseClickHigh);
                            //say("XX" xx);
                            #endif 
